...
def cluster(tree, instances, inst_size, minsplit=1, maxsplit=1, mod=True):
    """
    ...
    """
    for c in cluster_iter(tree, instances, inst_size, heuristic=CU, minsplit=minsplit,
...
def cluster_split_search(tree, instances, inst_size, heuristic=CU, minsplit=1, maxsplit=1,
    ...
    .. seealso:: :meth:`cluster_iter`
    """
    clus_it = cluster_iter(tree, instances, inst_size, heuristic=heuristic,
...
def cluster_iter(tree, instances, inst_size, heuristic=CU, minsplit=1, maxsplit=100000,
    ...
    # Obtain the leaf membership of each instance
    if mod:
        temp_clusters = [tree.ifit(instance) for instance in instances]
    else:
        temp_clusters = [tree.categorize(instance, greedy=True) for instance in instances]

    # Derive the assignment of each instance given the parameters
    for nth_split in range(1, maxsplit+1):
        ...
        if nth_split >= minsplit:
            ...
            print("cluster_assign", set(cluster_assign))
            if heuristic == CU:
                yield clusters, heuristic(cluster_assign, temp_clusters, inst_size)
            else:
        ...
        for i, target in enumerate(set(cluster_assign)):
            ...
            # c_labels = [label if (label != target or child_cluster_assign[j] is None) else child_cluster_assign[j]
            #             for j, label in enumerate(cluster_assign)]
            print("c_labels", set(c_labels))
            if heuristic == CU:
                split_cus.append((heuristic(c_labels, temp_clusters, inst_size), i, target))
            else:
